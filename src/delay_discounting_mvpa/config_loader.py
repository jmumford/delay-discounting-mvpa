...
class PathsConfig:
    ...
    def __post_init__(self):
        ...
        # Generate glob pattern for the BOLD file's mask
...
class Config:
    ...
    def __init__(self, config_file: str = 'config.yaml', validate: bool = True):
        ...
        -----------
        config_file : str
            Path or filename of the YAML configuration file to load.
            - Absolute paths are accepted.
            - Relative paths are first tried relative to current working dir.
            - If not found, the loader searches upward from this module to find
              the project root (where pyproject.toml lives) and will look in
              project_root / <config_file> and project_root / "configs" / <basename>.
        validate : bool
            Whether to validate the configuration after loading.
        """
        resolved_path = self._resolve_config_path(config_file)
        # store as string for backward-compatible representation
        self.config_file = str(resolved_path)
        self._raw_config = self._load_yaml_config(self.config_file)
        self._parse_config()
        if validate:
    ...
    def _resolve_config_path(self, config_file: str) -> Path:
        ...
        3. Search upward from this file for project root (pyproject.toml).
           Then try:
             - project_root / config_file
             - project_root / "configs" / basename(config_file)
        ...
        if candidate.is_absolute():
            ...
            raise ConfigError(f'Config file not found at absolute path: {candidate}')
        ...
        # 3) Find project root by looking for pyproject.toml upward from this file
        ...
        if project_root is None:
            raise ConfigError(
                ...
                'Cannot resolve config file.'
            )

        checked = []
        # prefer project_root / provided path (preserves subdirectories if provided)
        proj_candidate = project_root / candidate
        checked.append(str(proj_candidate))
        if proj_candidate.exists():
            return proj_candidate.resolve()

        # also try project_root / "configs" / basename
        configs_candidate = project_root / 'configs' / candidate.name
        checked.append(str(configs_candidate))
        if configs_candidate.exists():
            return configs_candidate.resolve()

        # not found — raise with helpful diagnostics
        raise ConfigError(
            f"Config file '{config_file}' not found. Checked locations:\n  - "
            + '\n  - '.join(checked)
        )
